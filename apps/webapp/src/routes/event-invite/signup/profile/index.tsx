...
const Profile = () => {
  ...
  const { data, loading: isLoadingInvite } = useQuery<GetEventAttendanceQuery>(
    GetEventAttendance,
    {
  ...
  const [updateUser, { loading: isLoadingUpdateUser }] =
    useMutation<UpdateAttendanceUserMutation>(Profile.mutations.updateUser, {
      update(cache) {
  ...
  if (!inviteId || !data?.eventAttendance) return <div>Not found</div>;
...
Profile.mutations = {
  updateUser: graphql(/* GraphQL */ `
    mutation UpdateAttendanceUser($input: UpdateUserInput!) {
...
export const Route = createFileRoute("/event-invite/signup/profile/")({
  component: Profile,
